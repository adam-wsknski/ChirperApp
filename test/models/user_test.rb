...
class UserTest < ActiveSupport::TestCase
  ...
  def setup
    @user = User.new(name: "Example User", email: "example@exp.com",
  ...
  test "email should be max 255 chars long" do
    @user.email = "a" * 250 + "@us.co"
    assert_not @user.valid?
  end
 ...
 test "associated microposts should be destroyed" do
   @user.save
   @user.microposts.create!(content: "tatata")
 ...
 test "feed should have the right posts" do
   ...
   # Posts from self
   adam.microposts.each do |post_following|
     assert adam.feed.include?(post_following)
   end
   # Posts from unfollowed user
   archer.microposts.each do |post_following|
     assert_not adam.feed.include?(post_following)
   end
 end
end
